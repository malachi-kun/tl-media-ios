...
public class TransitionGestureHandler : NSObject {
    ...
    public init(targetView: UIView, direction: DirectionType) {
     ...
     open func registerGesture(_ view: UIView) {
    ...
    func isNeedSetPanStartLocation() -> Bool {
     ...
     func setPanStartPoint(_ location: CGPoint) {
        switch self.direction {
        case .top:
     ...
     func forceCancelIfNeeded(_ location: CGPoint) {
        if !self.isTransitioning { return }
     ...
     func updatePercentComplete(_ location: CGPoint) {
     ...
     func startTransitionIfNeeded(_ location: CGPoint) {
        if self.isTransitioning { return }
